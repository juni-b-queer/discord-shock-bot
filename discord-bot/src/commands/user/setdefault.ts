...
export async function execute(
    ...
    const setDefault = await dependencies.dbClient.setUserDefaultShocker(
        guildUser,
        defaultShocker
    )
    ...
    if (!setDefault) {
        return await interaction.reply({
            content: 'Failed to set default shocker.',
    ...
    debugLog(
        'INFO',
        'setDefault',
        `Changed default shocker to ${defaultShocker} for ${interaction.user.username}`
    )
    await interaction.reply({
        content: `Set default shocker to ${defaultShocker}`,
        flags: MessageFlags.Ephemeral,
    })
}
