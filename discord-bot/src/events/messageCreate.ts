...
// @ts-ignore
export async function execute(dependencies: InteractionDeps, interaction) {
    ...
    if (interaction.guildId === null) {
        ...
        if (!dbUser) {
            interaction.author.send(
                'You are not registered. Please register by reacting to the setup message'
            )
        }
        // DM
        debugLog(
            'INFO',
            'messageCreate',
            `Got a DM from ${interaction.author.username}`
        )
        ...
        for (const shocker of shockers) {
            const existingShocker =
                await dependencies.database.query.shockersTable.findFirst({
            ...
            try {
                if (existingShocker) {
                    ...
                    await dependencies.database
                        .insert(shockersTable)
                        .values(shockerValues)
        ...
        if (parsedData.apiToken !== null) {
            await dependencies.database
                .update(usersTable)
                .set({ apiKey: parsedData.apiToken })
        ...
        debugLog(
            'INFO',
            'messageCreate',
            `Shocker setup for ${dbUser!.globalName}`
        )
