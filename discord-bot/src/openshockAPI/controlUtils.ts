...
export async function generateAndRunBasicControlRequests(
    ...
    if (!user.apiKey) {
        return await interaction.reply({
            content: 'User has not set up their shockers',
            flags: MessageFlags.Ephemeral,
        });
    }
    if (user.paused) {
        return await interaction.reply({
            content: 'This user has paused their shockers',
            flags: MessageFlags.Ephemeral,
        });
    }
    ...
    if (options.shocker === null) {
        const defaultShocker =
            await dependencies.dbClient.getUserDefaultShocker(user.userId);

        if (!defaultShocker) {
            user.shockers.forEach((shocker) =>
                shockerIds.push(shocker.shockerId)
            );
        } else {
            shockerIds.push(defaultShocker.shockerId);
        }
    } else {
        const selectedShocker = user.shockers.find(
            (shocker) => shocker.name === options.shocker
        )!;
...
export function sequenceParser(
    shockerId: string,
    sequenceString: string,
    userIntensityLimit: number
    ...
    for (const command of commands) {
        ...
        const osAction: 'Shock' | 'Vibrate' | 'Stop' =
            action === 's' ? 'Shock' : action === 'v' ? 'Vibrate' : 'Stop';

        let setIntensity = parseInt(intensity);
        if (osAction === 'Shock' && setIntensity > userIntensityLimit) {
            setIntensity = userIntensityLimit;
        }
